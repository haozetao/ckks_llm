...
namespace RNG
{
    ...
    __constant__ uint8_tt MY_ALIGN(sizeof(uint32_t)) key[XSALSA20_CRYPTO_KEYBYTES * THREADS_PER_BLOCK];
    __constant__ uint8_tt MY_ALIGN(sizeof(uint32_t)) sigma[16];
    ...
    __host__ __device__ static inline uint32_t
    load_littleendian(const uint8_tt* x)
    {
        return
            (uint32_t)(x[0]) \
            | (((uint32_t)(x[1])) << 8) \
    ...
    load_littleendian64(const uint8_tt* x)
    {
        return
            (uint64_t)(x[0]) \
            | (((uint64_t)(x[1])) << 8) \
    ...
    __global__ static void VecCrypt(uint8_tt* A, uint32_tt N, uint64_t nblocks, uint64_t p_nonce, int blks_per_chunk)
    {
        ...
        if (i < N) {
            ...
            for (k = 0; k < tot; k++) {
                ...
                mem = (uint32_tt*)&A[blockno * XSALSA20_BLOCKSZ];
                *mem ^= x0;  mem++;
                *mem ^= x1;  mem++;
                *mem ^= x2;  mem++;
                *mem ^= x3;  mem++;
