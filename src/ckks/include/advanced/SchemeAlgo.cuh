...
void SchemeAlgo::malloc_bsgs_buffer(int sine_degree)
{
    int t_num = context.t_num;
    int Ri_blockNum = context.Ri_blockNum;
    int Qj_blockNum = context.Qj_blockNum;

    int sine_tree_level = ceil(log2(sine_degree));

    int tree_node_num = 1<<int(ceil(log2(sine_degree)));
    ...
    for(int i = 0; i < ceil(log2(sine_degree)); i++)
    {
